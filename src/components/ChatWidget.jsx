...
/**
 * Preset FAQ items for quick help options
 */
const QUICK_HELP_OPTIONS = [
  ...
  { q: 'What are the prize rewards?', a: 'Prize pool includes ₹3,000 (1st), ₹2,000 (2nd), ₹1,000 (3rd), plus internships & swag kits!' },
  { q: 'When is the hackathon?', a: 'Registration opens Oct 15, and the 48-hour hackathon starts Dec 01, 2026.' }
];
...
const ChatWidget = () => {
  ...
  const handleSend = (textToSend) => {
    ...
    setTimeout(() => {
      let reply = "Thanks for asking! Check out our Domains, Timeline, and FAQ sections for full event details, or register your team now!";
          ...
          >
            ...
            <div className="p-4 bg-gradient-to-r from-cyan-500/20 via-blue-600/20 to-purple-600/20 border-b border-white/10 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="relative p-2 rounded-xl bg-cyan-500/20 text-cyan-400 border border-cyan-500/30 shadow-[0_0_12px_rgba(0,229,255,0.3)]">
                  <Bot className="w-5 h-5" />
                  <span className="absolute -top-1 -right-1 w-2.5 h-2.5 rounded-full bg-emerald-400 animate-pulse" />
                </div>
                <div>
                  <h4 className="text-sm font-bold text-white tracking-tight flex items-center gap-1.5">
                    ZAYATHON AI Support <Sparkles className="w-3.5 h-3.5 text-cyan-400" />
                  </h4>
                  <span className="text-[10px] text-emerald-400 font-medium">Online • Instant Answers</span>
                </div>
              </div>
            ...
            <div className="flex-1 p-4 overflow-y-auto space-y-3 text-xs">
              ...
              {messages.length < 5 && (
                <div className="pt-2">
                  <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider block mb-2 flex items-center gap-1">
                    <HelpCircle className="w-3 h-3 text-cyan-400" /> Quick Help Options:
                  </span>
                  <div className="flex flex-wrap gap-1.5">
                    {QUICK_HELP_OPTIONS.map((item, qIdx) => (
                      <button
                        key={qIdx}
